# Catálogos y niveles de riesgo

Niveles 1–5 por variable. Fuente: catálogo simplificado operable derivado de la Base de Variables BX+ (claves serie 9000 para factores fiduciarios).

## Tipo de persona (F18)

| Variable | Nivel |
|---|---|
| PF nacional | 1 |
| PF con actividad empresarial | 2 |
| PM nacional | 3 |
| Mandato | 4 |
| PM extranjera | 5 |
| Fideicomiso / vehículo | 5 |

## Actividad económica (F12)

| Variable | Nivel |
|---|---|
| Sin actividad de riesgo / asalariado | 1 |
| Servicios profesionales | 2 |
| Comercio general | 2 |
| Sector financiero regulado | 3 |
| No capturada | 3 |
| Inmobiliario / construcción | 4 |
| Comercio exterior | 4 |
| Alto manejo de efectivo (casas de empeño, gasolineras, restaurantes, menudeo) | 5 |

## País (F4/F5/F6 — nacimiento, nacionalidad, domicilio)

| Variable | Nivel |
|---|---|
| México | 1 |
| Norteamérica / Europa (bajo riesgo) | 2 |
| Otro país | 3 |
| Régimen fiscal preferente | 4 |
| Lista GAFI gris | 5 |
| Lista GAFI negra / sancionado | 5 |

## Estado del domicilio (F7)

| Variable | Nivel |
|---|---|
| Baja incidencia delictiva | 1 |
| Incidencia media | 2 |
| Domicilio en el extranjero | 3 |
| Alta incidencia delictiva | 4 |
| Frontera / puerto | 4 |

## Edad / antigüedad (F2)

| Variable | Nivel |
|---|---|
| Más de 10 años | 1 |
| 3 a 10 años | 2 |
| 1 a 3 años | 4 |
| Menos de 1 año | 5 |

## Rol en el fideicomiso (FIR)

| Variable | Nivel |
|---|---|
| Fideicomisario | 3 |
| Apoderado / delegado | 2 |
| Tercero administrador / garante / depositario | 1 |
| Fideicomitente | 4 |
| Comité técnico | 4 |
| Fideicomitente-Fideicomisario | 5 |

## Tipología del fideicomiso (FF1)

| Variable | Nivel |
|---|---|
| Administración | 2 |
| Garantía | 2 |
| Testamentario / planeación patrimonial | 2 |
| Inversión | 3 |
| Fuente de pago | 3 |
| Recibido por sustitución fiduciaria | 4 |
| Zona restringida | 4 |
| Traslativo de dominio | 5 |

## Patrimonio aportado (FF2)

| Variable | Nivel |
|---|---|
| Efectivo (numerario) / valores bursátiles cotizados en custodia | 1 |
| Inmuebles y bienes muebles (vehículos, arte, joyas, maquinaria) | 3 |
| Mixto (efectivo en numerario + especie) | 3 |
| Acciones, partes sociales y otros títulos no cotizados | 4 |
| Derechos (de cobro, fideicomisarios, etc.) / otros bienes | 4 |

## Valor del patrimonio (FF3)

| Variable | Nivel |
|---|---|
| Hasta 5 MDP | 1 |
| 5 a 20 MDP | 2 |
| 20 a 100 MDP | 3 |
| 100 a 500 MDP | 4 |
| Más de 500 MDP | 5 |

## Antigüedad del fideicomiso (FF4)

| Variable | Nivel |
|---|---|
| Más de 10 años | 1 |
| 3 a 10 años | 2 |
| 1 a 3 años | 4 |
| Menos de 1 año | 5 |

## Actividad Vulnerable LFPIORPI (FF6)

| Variable | Nivel |
|---|---|
| Sin actividad vulnerable (Tipo A) | 1 |
| En dictamen (Tipo C) | 3 |
| AV confirmada Art. 17 (Tipo B) | 5 |

## Procedencia de recursos (FF7)

| Variable | Nivel |
|---|---|
| Bancarizados con soporte (ahorros, dividendos, salarios, etc.) | 1 |
| Venta de activos documentada (inmuebles, acciones, etc.) | 2 |
| Aportación en especie sin avalúo | 4 |
| Origen no verificable | 5 |

## USD / transfronterizo (FF8)

| Variable | Nivel |
|---|---|
| Sin USD ni parte extranjera | 1 |
| USD ocasional / parte extranjera | 3 |
| Flujos recurrentes o jurisdicción de riesgo | 5 |

## Desviación del fin (FF9)

| Variable | Nivel |
|---|---|
| Opera conforme al contrato / aún no evaluado | 1 |
| Operaciones poco usuales para el fin del contrato | 4 |
| Operaciones fuera del objeto del contrato | 5 |

